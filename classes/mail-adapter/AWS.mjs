...
export default class MailAdapterAWSSES extends MailAdapter {
    ...
    async send(subject, text, sender, recipient, opts = {}) {
        const { cc = '', bcc = '', html = '', attachments = [], 
        // metadata = [],
        project = Central.config.mail.aws.project, dynamoDB = Central.config.mail.aws.dynamoDB, configurationSetName = Central.config.mail.aws.configurationSetName, reply_to = '' } = opts;
        const mail = {
            from: sender,
            to: recipient,
            subject: subject,
    ...
    async readLog(email) {
        ...
        // fetch from dynamoDB
        const results = await MailAdapterAWSSES.ddb.send(new ScanCommand({
        ...
        results.Items?.forEach(item => {
            ...
            if (!mailId)
                return;
            if (!mailLogs.has(mailId))
                mailLogs.set(mailId, { open: 0, click: 0 });
