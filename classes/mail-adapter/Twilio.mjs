...
export default class MailAdapterTwilio extends MailAdapter {
    ...
    constructor(opts = {}) {
        super();
        this.service = 'Twilio';
        this.client = Twilio(Central.adapter.process().env.TWILIO_SID, Central.adapter.process().env.TWILIO_TOKEN);
    }
    /**
     *
    ...
    async send(subject, text, sender, recipient, opts = {}) {
        ...
        const results = await Promise.all(recipients.map(async (it) => {
